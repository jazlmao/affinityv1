-- Affinity V1: Restored | Murderers VS Sheriffs Duels
-- (Affinity SCRIPT HUB IS ONLY IN ALPHA PHASE AND IS UNFINISHED! EXPECT MANY BUGS WHEN EXPERIENCING WITH THIS UTILITY!)

if not shared.AffinityExecuted then
	game:Shutdown()
end
...
local window = lib:CreateWindow({
	Name="Affinity V1 | MVS Duels",
	LoadingTitle="MVS Duels Script Utility",
	LoadingSubtitle="by: jazlmao. & Saber#4705 ",
	ConfigurationSaving={
					...
					if customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicRoundEnded.CurrentValue and string.lower(result) == "won" then
								funcs.sayMessage(autoToxicRoundEndedPhrase ~= "" and autoToxicRoundEndedPhrase or "ez win LLL | Affinity V1")
							end
						end
					end
				end))
					...
					if customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicMatchEnded.CurrentValue and string.lower(result) == "won" then
								funcs.sayMessage(autoToxicMatchEndedPhrase ~= "" and autoToxicMatchEndedPhrase or "EZZZZZ | Affinity V1")
							end
						end
					end
				end))
					...
					if owner == customStore.you.lplr and customStore.you.status.inMatch then
						if not chatSpamBlockingAutoToxic then
							if autoToxicPlayerKilled.CurrentValue and funcs.getTeamStatus(player) == "Enemy" then
								local message = autoToxicPlayerKilledPhrase ~= "" and autoToxicPlayerKilledPhrase or "L <name> | Affinity V1"
								if message then
	...
	chatSpammer = playerTab:CreateToggle({
		...
		Callback = function(value)
			...
			if value then
				task.spawn(function()
					repeat
						funcs.sayMessage(chatSpammerMessage or "Affinity V1 on top W")
...
task.spawn(function()
	...
	nametagsUI.Name = "AffinityNametagsUI"
...
if whitelist.ClientPriority > 0 then miscTab:CreateSection("Trolling Configs") end
task.spawn(function()
...
-- Affinity user detection: [testing]
ranxConnections:BindToHeartbeat("AffinityUserDetection",function()
	for _,plr in next,game.Players:GetPlayers() do
		if whitelist.ClientPriority > 0 then
			if plr ~= customStore.you.lplr then
				...
				if not whitelisted then
					if plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChildOfClass("Humanoid").HipHeight > 2.4 and not table.find(shared.ranxUsers, plr) then
						...
						lib:Notify({
							Title = "Affinity V1 Whitelist",
							Content = "@" .. plr.Name .. " has been detected using Affinity V1!",
							Duration = 8,
						})
					end
				end
...
ranxConnections:BindToHeartbeat("AffinityModeratorDetection",function()
	for _,plr in next,game.Players:GetPlayers() do
		if plr ~= customStore.you.lplr then
